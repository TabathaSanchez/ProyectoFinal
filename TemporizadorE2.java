...
public class TemporizadorE2 extends JFrame implements ActionListener{
	...
	ThreadTemporizadorE2 threadt;

	public TemporizadorE2(){
		initialize();
		setSize(228,150);
	...
	private void finalizarHiloTemporizador(){
		if(minuto==0&&segundo==0&&centesimasegundo==0){
			try{threadt.stop();}catch(Exception e){e.printStackTrace();}
	...
	private void iniciarHiloTemporizador(){
		if(iniciarHilo==true){
			threadt= new ThreadTemporizadorE2(label);
			threadt.start();
			if(finalizado==true){
